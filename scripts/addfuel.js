...
// File path
const filePath = "./results/fuel.json";
...
if (fs.existsSync(filePath)) {
  try {
    ...
    console.error("Invalid JSON in fuels file.");
    process.exit(1);
  }
}
...
switch (userSelect) {
  case "1":{
    ...
    const collection = "Fuel"
  
const equipmentName = `${siteName} Fuel`;
...
const equipmentID = network == "SLERS"
  ? `${selectedSite.altSiteName}XF`
  : `${selectedSite.siteNumber}XF`;
  
    const serialNumber = prompt("Enter the serial number:")
    const make = prompt("Enter the manufacturer:")
    const model = prompt("Enter the model or part number:")
    const capacity = prompt("Enter the size/capacity:")
...
Object.keys(newEntry).forEach(key => {
  ...
  break
    ...
    case "2":{
    const type = "Fuel Tank";
    const caseType = "Work Order";
    const collection = "Fuel"
  

const equipmentName = `${siteName} Fuel Tank`;

const equipmentID = network == "SLERS"
  ? `${selectedSite.altSiteName}XFT`
  : `${selectedSite.siteNumber}XFT`;
  
    const serialNumber = prompt("Enter the serial number:")
    const make = prompt("Enter the manufacturer:")
    const model = prompt("Enter the model or part number:")
    const capacity = prompt("Enter the size/capacity:")
    const fuelType = prompt("Fuel Type: ").trim();
    const fuelHistory = [
      {
        recordDate: null,
        recordCaseNumber: null,
        recordType: null,
        previousPercentageAmount: null,
        currentPercentageAmount: null,
        gallonsFilled: null,
        lastFillVendor: null
      }
    ]
    const refuelNeeded = true;
