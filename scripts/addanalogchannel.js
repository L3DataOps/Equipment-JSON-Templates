...
/*
      {
        ...
        "equipmentName": "SiteName Analog Channel X",
...
if (network == "SLERS") {
  ...
  const type = "Analog Channel";
  const caseType = "Ticket";
  const collection = "Channels";

  const num = prompt("Enter which number for the for the analog channel (Ex. 1 = Analog Channel 1): ").trim();

  const equipmentName = `${siteName} Analog Channel ${num}`;
  const num2 = prompt("Enter the AMMS EQ Number ECPAX or CCPAX:")
  const equipmentID = `${selectedSite.altSiteName}${num2}`;
...
const email = [
  ...
  const equipmentName = `${siteName} Conventional Analog Voter ${num}`;
  const num2 = prompt("Enter the number after the CCPA in the equipment number for SLERS 2 EQ:")
  const equipmentID = `${selectedSite.siteNumber}CCPA${num2}`;
